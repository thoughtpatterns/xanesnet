...
def train_aegan(
    ...
    loss_gen_reg_type = hyperparams["loss_gen"]["loss_reg_type"]
    loss_gen_reg = True if loss_gen_reg_type is not None else False
    lambda_gen_reg = hyperparams["loss_gen"]["loss_reg_param"]

    # Regularisation of dis loss function
    loss_dis_reg_type = hyperparams["loss_dis"]["loss_reg_type"]
    loss_dis_reg = True if loss_gen_reg_type is not None else False
    lambda_dis_reg = hyperparams["loss_dis"]["loss_reg_param"]
    ...
    with mlflow.start_run(experiment_id=EXPERIMENT_ID, run_name=RUN_NAME):
        ...
        for epoch in range(n_epoch):
            ...
            for inputs_x, inputs_y in trainloader:
                ...
                if loss_gen_reg:
                    gen_a_l_reg = model_utils.loss_reg_fn(model.gen_a, loss_gen_reg_type, device)
                    gen_b_l_reg = model_utils.loss_reg_fn(model.gen_b, loss_gen_reg_type, device)
                    loss_gen_total += lambda_gen_reg * (gen_a_l_reg + gen_b_l_reg)

                # Regularisation of dis loss
                if loss_dis_reg:
                    dis_a_l_reg = model_utils.loss_reg_fn(model.dis_a, loss_dis_reg_type, device)
                    dis_b_l_reg = model_utils.loss_reg_fn(model.dis_b, loss_dis_reg_type, device)
                    loss_dis_total += lambda_dis_reg * (dis_a_l_reg + dis_b_l_reg)
