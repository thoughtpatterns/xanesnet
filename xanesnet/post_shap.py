...
def run_shap_gnn(model, save_path, data, index, n_samples=100, shap_mode="predict"):
    ...
    background_input = [
        torch.stack([data[i].graph_attr for i in bg_idxs]).float().to(device),         # (n_samples, n_features)
        torch.tensor(bg_idxs, dtype=torch.float32, device=device).unsqueeze(1)
    ]

    # Full dataset we want to explain
    input_to_explain = [
        torch.stack([g.graph_attr for g in data]).float().to(device),                  # (n_graphs, n_features)
