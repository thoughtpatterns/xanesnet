...
class AE(nn.Module):
    def __init__(self, input_size, hidden_size, dropout, hl_shrink, out_dim, n_class):
        ...
        self.encoder_hidden_2 = nn.Sequential(nn.Linear(512, 256), nn.PReLU())

        self.encoder_output = nn.Sequential(
            nn.Linear(256, 128),
        )

        self.fc1 = nn.Sequential(nn.Linear(128, 64), nn.Dropout(p=0.2), nn.PReLU())
        ...
        self.decoder_hidden_1 = nn.Sequential(nn.Linear(128, 256), nn.PReLU())

        self.decoder_hidden_2 = nn.Sequential(nn.Linear(256, 512), nn.PReLU())

        self.decoder_output = nn.Sequential(nn.Linear(512, self.input_size))
